/**
 * Phase 37 — Score Band Outcome Analysis Models v1: validation and safety.
 */
...
import {
  ...
  type ScoreBandOutcomeSafetyResult,
  type ScoreBandOutcomeValidationIssue,
  type ScoreBandOutcomeValidationResult,
} from './types.js';
import {
  isScoreBandOutcomeAnalysisFixtureSerializable,
  normalizeScoreBandOutcomeAnalysisFixture,
} from './normalization.js';
import { REPLAY_OUTCOME_FIXTURE_NAMES, REPLAY_OUTCOME_FIXTURE_KINDS } from '../replay-outcomes/types.js';
...
const REAL_PNL_PATTERN =
  /\b(?:real pnl|live pnl|balance|order fill|filled order|paper trade|live trade)\b/i;
...
): ScoreBandOutcomeValidationIssue {
...
function validateSortedStrings(
  ...
  field: string,
): readonly ScoreBandOutcomeValidationIssue[] {
...
function validateMeta(
  ...
  const issues: ScoreBandOutcomeValidationIssue[] = [];
  const { meta } = fixture;

  if (meta.phase !== 37)
    issues.push(makeIssue('INVALID_PHASE', 'meta.phase must be 37.', 'meta.phase'));
  if (meta.generatedAt !== PHASE_37_SCORE_BAND_OUTCOMES_GENERATED_AT) {
    issues.push(
      makeIssue(
        'NON_DETERMINISTIC_GENERATED_AT',
        'meta.generatedAt must be the fixed Phase 37 constant.',
        'meta.generatedAt',
      ),
    );
  }
  if (meta.source !== PHASE_37_SCORE_BAND_OUTCOMES_SOURCE) {
    issues.push(
      makeIssue(
        'INVALID_SOURCE',
        'meta.source must be the Phase 37 source constant.',
        'meta.source',
      ),
    );
  }
  if (meta.fixtureOnly !== true)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.fixtureOnly must be true.', 'meta.fixtureOnly'),
    );
  if (meta.syntheticOnly !== true)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.syntheticOnly must be true.', 'meta.syntheticOnly'),
    );
  if (meta.deterministic !== true)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.deterministic must be true.', 'meta.deterministic'),
    );
  ...
  if (meta.inMemoryOnly !== true)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.inMemoryOnly must be true.', 'meta.inMemoryOnly'),
    );
  ...
  if (meta.realScoring !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.realScoring must be false.', 'meta.realScoring'),
    );
  if (meta.realBacktesting !== false)
    issues.push(
      makeIssue(
        'SAFETY_VIOLATION',
        'meta.realBacktesting must be false.',
        'meta.realBacktesting',
      ),
    );
  if (meta.paperTrading !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.paperTrading must be false.', 'meta.paperTrading'),
    );
  if (meta.liveTrading !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.liveTrading must be false.', 'meta.liveTrading'),
    );
  if (meta.execution !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.execution must be false.', 'meta.execution'),
    );
  ...
  if (meta.persistence !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.persistence must be false.', 'meta.persistence'),
    );
  if (meta.fileExport !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.fileExport must be false.', 'meta.fileExport'),
    );
  if (meta.nonAdvisory !== true)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'meta.nonAdvisory must be true.', 'meta.nonAdvisory'),
    );
...
function validateSummary(
  ...
  if (summary.phase !== 37)
    issues.push(makeIssue('INVALID_PHASE', 'summary.phase must be 37.', 'summary.phase'));
  if (summary.name !== fixture.name)
    issues.push(
      makeIssue('NAME_MISMATCH', 'summary.name must match fixture.name.', 'summary.name'),
    );
  if (summary.kind !== fixture.kind)
    issues.push(
      makeIssue('KIND_MISMATCH', 'summary.kind must match fixture.kind.', 'summary.kind'),
    );
  if (summary.generatedAt !== PHASE_37_SCORE_BAND_OUTCOMES_GENERATED_AT) {
    issues.push(
      makeIssue(
        'NON_DETERMINISTIC_GENERATED_AT',
        'summary.generatedAt must be fixed Phase 37 constant.',
        'summary.generatedAt',
      ),
    );
  }
  ...
  if (summary.syntheticOnly !== true)
    issues.push(
      makeIssue(
        'SAFETY_VIOLATION',
        'summary.syntheticOnly must be true.',
        'summary.syntheticOnly',
      ),
    );
  if (summary.localOnly !== true)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'summary.localOnly must be true.', 'summary.localOnly'),
    );
  if (summary.liveData !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'summary.liveData must be false.', 'summary.liveData'),
    );
  if (summary.realScoring !== false)
    issues.push(
      makeIssue(
        'SAFETY_VIOLATION',
        'summary.realScoring must be false.',
        'summary.realScoring',
      ),
    );
  ...
  if (summary.paperTrading !== false)
    issues.push(
      makeIssue(
        'SAFETY_VIOLATION',
        'summary.paperTrading must be false.',
        'summary.paperTrading',
      ),
    );
  if (summary.liveTrading !== false)
    issues.push(
      makeIssue(
        'SAFETY_VIOLATION',
        'summary.liveTrading must be false.',
        'summary.liveTrading',
      ),
    );
  if (summary.execution !== false)
    issues.push(
      makeIssue('SAFETY_VIOLATION', 'summary.execution must be false.', 'summary.execution'),
    );
  ...
  if (typeof summary.riskCount !== 'number' || summary.riskCount < 0) {
    issues.push(
      makeIssue(
        'INVALID_COUNT',
        'summary.riskCount must be non-negative.',
        'summary.riskCount',
      ),
    );
  }
  ...
  if (
    !(REPLAY_OUTCOME_FIXTURE_NAMES as readonly string[]).includes(
      summary.referencedReplayOutcomeFixtureName,
    )
  ) {
    issues.push(
      makeIssue(
        'INVALID_PHASE36_REF',
        'summary.referencedReplayOutcomeFixtureName must be a valid Phase 36 fixture name.',
        'summary.referencedReplayOutcomeFixtureName',
      ),
    );
  }
...
function validateOutcomeReference(
  fixture: ScoreBandOutcomeAnalysisFixture,
): readonly ScoreBandOutcomeValidationIssue[] {
  const issues: ScoreBandOutcomeValidationIssue[] = [];
  const { outcomeReference } = fixture;

  if (
    !(REPLAY_OUTCOME_FIXTURE_NAMES as readonly string[]).includes(
      outcomeReference.replayOutcomeFixtureName,
    )
  ) {
    issues.push(
      makeIssue(
        'INVALID_PHASE36_REF',
        'outcomeReference.replayOutcomeFixtureName is invalid.',
        'outcomeReference.replayOutcomeFixtureName',
      ),
    );
  }
  if (
    !(REPLAY_OUTCOME_FIXTURE_KINDS as readonly string[]).includes(
      outcomeReference.replayOutcomeFixtureKind,
    )
  ) {
    issues.push(
      makeIssue(
        'INVALID_PHASE36_KIND_REF',
        'outcomeReference.replayOutcomeFixtureKind is invalid.',
        'outcomeReference.replayOutcomeFixtureKind',
      ),
    );
  }
  if (
    !(SCORE_BAND_RANGE_CATEGORIES as readonly string[]).includes(
      outcomeReference.scoreBandCategory,
    )
  ) {
    issues.push(
      makeIssue(
        'INVALID_SCORE_BAND_CATEGORY',
        'outcomeReference.scoreBandCategory is invalid.',
        'outcomeReference.scoreBandCategory',
      ),
    );
  }
  issues.push(...validateSortedStrings(outcomeReference.notes, 'outcomeReference.notes'));

  return issues;
}

export function validateScoreBandOutcomeAnalysisFixture(
  fixture: unknown,
): ScoreBandOutcomeValidationResult {
  if (!isRecord(fixture)) {
    return {
      valid: false,
  ...
  issues.push(
    ...validateRequiredString(
      fixture['name'],
      'name',
      SCORE_BAND_OUTCOME_ANALYSIS_FIXTURE_NAMES as unknown as readonly string[],
    ),
  );
  issues.push(
    ...validateRequiredString(
      fixture['kind'],
      'kind',
      SCORE_BAND_OUTCOME_ANALYSIS_FIXTURE_KINDS as unknown as readonly string[],
    ),
  );
  ...
  if (!isRecord(fixture['meta'])) {
    ...
    issues.push(...validateMeta(fixture as unknown as ScoreBandOutcomeAnalysisFixture));
  }

  if (!isRecord(fixture['summary'])) {
    issues.push(makeIssue('MISSING_REQUIRED_FIELD', 'summary is required.', 'summary'));
  } else {
    issues.push(...validateSummary(fixture as unknown as ScoreBandOutcomeAnalysisFixture));
  }

  if (!isRecord(fixture['outcomeReference'])) {
    issues.push(
      makeIssue(
        'MISSING_REQUIRED_FIELD',
        'outcomeReference is required.',
        'outcomeReference',
      ),
    );
  } else {
    issues.push(
      ...validateOutcomeReference(fixture as unknown as ScoreBandOutcomeAnalysisFixture),
    );
  }

  if (!isRecord(fixture['scoreBandRange'])) {
    issues.push(
      makeIssue('MISSING_REQUIRED_FIELD', 'scoreBandRange is required.', 'scoreBandRange'),
    );
  }

  if (!isRecord(fixture['outcomeDistribution'])) {
    issues.push(
      makeIssue(
        'MISSING_REQUIRED_FIELD',
        'outcomeDistribution is required.',
        'outcomeDistribution',
      ),
    );
  }
  ...
  if (!Array.isArray(fixture['riskIndicators'])) {
    issues.push(
      makeIssue(
        'MISSING_REQUIRED_FIELD',
        'riskIndicators must be an array.',
        'riskIndicators',
      ),
    );
  } else {
    issues.push(
      ...validateIndicatorOrdering(
        fixture['riskIndicators'] as { code: string }[],
        'riskIndicators',
      ),
    );
  }
  ...
  if (!Array.isArray(fixture['safeNotes'])) {
    issues.push(
      makeIssue('MISSING_REQUIRED_FIELD', 'safeNotes must be an array.', 'safeNotes'),
    );
  } else {
    issues.push(...validateSortedStrings(fixture['safeNotes'] as string[], 'safeNotes'));
  }

  if (!isScoreBandOutcomeAnalysisFixtureSerializable(fixture)) {
    issues.push(makeIssue('NOT_SERIALIZABLE', 'Fixture must be JSON-serializable.', 'root'));
  }

  if (issues.length === 0) {
    const normalized = normalizeScoreBandOutcomeAnalysisFixture(
      fixture as unknown as ScoreBandOutcomeAnalysisFixture,
    );
...
export function validateScoreBandOutcomeSafety(fixture: unknown): ScoreBandOutcomeSafetyResult {
  if (fixture === null || fixture === undefined) {
    return {
      safe: false,
      violations: ['Score-band outcome safety validation requires a non-null object.'],
    };
  }
