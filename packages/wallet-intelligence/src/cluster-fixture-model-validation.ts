/**
 * Phase 31 — Wallet Cluster Fixture Models v1: validation and safety.
 */
...
import {
  ...
  type WalletClusterIntelligenceSafetyResult,
  type WalletClusterIntelligenceValidationIssue,
  type WalletClusterIntelligenceValidationResult,
...
} from './cluster-fixture-model-normalization.js';
...
const LIVE_DATA_PATTERN =
  /\b(?:live data|real-time|scraped|api collected|twitter api|x api|telegram api|discord api|wallet ownership|wallet owner)\b/i;
...
const EXECUTION_PATTERN =
  /\b(?:execute trade|place order|swap now|send transaction|sign transaction)\b/i;
...
): WalletClusterIntelligenceValidationIssue {
...
function validateSortedStrings(
  ...
  field: string,
): readonly WalletClusterIntelligenceValidationIssue[] {
...
function validateForbiddenContent(
  ...
  const issues: WalletClusterIntelligenceValidationIssue[] = [];
  const patterns: ReadonlyArray<[RegExp, string, string]> = [
    ...
    [
      ...
      'stack-trace-like content is forbidden in fixture models.',
    ],
    [
      LOCAL_PATH_PATTERN,
      'LOCAL_PATH_FORBIDDEN',
      'local filesystem paths are forbidden in fixture models.',
    ],
    [SECRET_PATTERN, 'SECRET_FORBIDDEN', 'secret-like content is forbidden in fixture models.'],
    [
      LIVE_DATA_PATTERN,
      'LIVE_DATA_CLAIM_FORBIDDEN',
      'live-data or scraping claims are forbidden in fixture models.',
    ],
    [
      ADVICE_PATTERN,
      'ADVICE_LANGUAGE_FORBIDDEN',
      'investment-advice language is forbidden in fixture models.',
    ],
    [
      EXECUTION_PATTERN,
      'EXECUTION_LANGUAGE_FORBIDDEN',
      'trading or execution language is forbidden in fixture models.',
    ],
  ];
...
export function validateWalletClusterIntelligenceSafety(
  value: unknown,
): WalletClusterIntelligenceSafetyResult {
  if (!value || typeof value !== 'object') {
    return {
      safe: false,
      violations: [
        'Wallet cluster fixture safety validation requires a non-null object.',
      ],
    };
  }
...
export function validateWalletClusterIntelligenceFixture(
  value: unknown,
): WalletClusterIntelligenceValidationResult {
  const issues: WalletClusterIntelligenceValidationIssue[] = [];
    ...
    return {
      ...
      issues: [
        makeIssue(
          ...
          'Wallet cluster fixture must be a non-null object.',
          'fixture',
        ),
      ],
    };
  ...
  const fixture = value as WalletClusterIntelligenceFixture;

  if (!WALLET_CLUSTER_INTELLIGENCE_FIXTURE_NAMES.includes(fixture.name)) {
    issues.push(makeIssue('UNSUPPORTED_FIXTURE_NAME', 'Fixture name is not supported.', 'name'));
  }
  if (!WALLET_CLUSTER_INTELLIGENCE_FIXTURE_KINDS.includes(fixture.kind)) {
    issues.push(makeIssue('UNSUPPORTED_FIXTURE_KIND', 'Fixture kind is not supported.', 'kind'));
  }
  if (fixture.meta.phase !== 31) {
    issues.push(makeIssue('INVALID_PHASE', 'meta.phase must be 31.', 'meta.phase'));
  }
  if (fixture.meta.generatedAt !== PHASE_31_WALLET_CLUSTER_GENERATED_AT) {
    issues.push(
      makeIssue(
        'INVALID_GENERATED_AT',
        'meta.generatedAt must use the deterministic Phase 31 timestamp.',
        'meta.generatedAt',
      ),
    );
  }
  if (fixture.meta.source !== PHASE_31_WALLET_CLUSTER_SOURCE) {
    issues.push(
      makeIssue(
        'INVALID_SOURCE',
        'meta.source must use the deterministic Phase 31 source tag.',
        'meta.source',
      ),
    );
  }
  if (fixture.meta.fixtureOnly !== true) {
    issues.push(
      makeIssue('FIXTURE_ONLY_REQUIRED', 'meta.fixtureOnly must be true.', 'meta.fixtureOnly'),
    );
  }
  if (fixture.meta.syntheticOnly !== true) {
    issues.push(
      makeIssue(
        ...
        'meta.syntheticOnly must be true.',
        'meta.syntheticOnly',
      ),
    );
  }
  if (fixture.meta.liveData !== false) {
    issues.push(
      makeIssue('LIVE_DATA_FORBIDDEN', 'meta.liveData must be false.', 'meta.liveData'),
    );
  }
  ...
  if (fixture.meta.readOnly !== true) {
    issues.push(
      makeIssue('READ_ONLY_REQUIRED', 'meta.readOnly must be true.', 'meta.readOnly'),
    );
  }
  if (fixture.meta.nonAdvisory !== true) {
    issues.push(
      makeIssue(
        'NON_ADVISORY_REQUIRED',
        'meta.nonAdvisory must be true.',
        'meta.nonAdvisory',
      ),
    );
  }
  if (!fixture.profile.clusterId || !fixture.profile.displayLabel) {
    issues.push(
      makeIssue(
        'REQUIRED_FIELDS_MISSING',
        'Fixture profile identifiers are required.',
        'profile',
      ),
    );
  }
  if (!isWalletClusterIntelligenceFixtureSerializable(fixture)) {
    issues.push(
      makeIssue('NOT_SERIALIZABLE', 'Fixture must be serializable.', 'fixture'),
    );
  }
  ...
  issues.push(...validateSortedStrings(fixture.summary.topQualityCodes, 'summary.topQualityCodes'));
  ...
  issues.push(...validateSortedStrings(fixture.history.notes, 'history.notes'));
  ...
  if (JSON.stringify(fixture) !== JSON.stringify(normalized)) {
    issues.push(
      makeIssue(
        'NOT_NORMALIZED',
        'Fixture must already be normalized and deterministically ordered.',
        'fixture',
      ),
    );
  }

  if (fixture.summary.phase !== 31) {
    issues.push(
      makeIssue('SUMMARY_PHASE_INVALID', 'summary.phase must be 31.', 'summary.phase'),
    );
  }
  if (fixture.summary.name !== fixture.name) {
    issues.push(
      makeIssue('SUMMARY_NAME_MISMATCH', 'summary.name must match fixture name.', 'summary.name'),
    );
  }
  if (fixture.summary.kind !== fixture.kind) {
    issues.push(
      makeIssue('SUMMARY_KIND_MISMATCH', 'summary.kind must match fixture kind.', 'summary.kind'),
    );
  }
  if (fixture.summary.generatedAt !== fixture.meta.generatedAt) {
    issues.push(
      makeIssue(
        'SUMMARY_GENERATED_AT_MISMATCH',
        'summary.generatedAt must match meta.generatedAt.',
        'summary.generatedAt',
      ),
    );
  }
  if (
    fixture.summary.fixtureOnly !== true ||
    fixture.summary.liveData !== false ||
    fixture.summary.externalNetwork !== false
  ) {
    issues.push(
      makeIssue(
        'SUMMARY_SAFETY_INVALID',
        'summary safety flags must remain fixture-only, local, and offline.',
        'summary',
      ),
    );
  }
