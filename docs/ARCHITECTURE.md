...
- `offline-intelligence`: Phase 33 composite fixture-model layer + Phase 34 report-integration layer + Phase 36 replay-outcome fixture layer; combines synthetic cross-references to Phase 30/31/32 fixture outputs into 16 composite evidence fixtures, transforms them into 16 report-integration fixtures, then maps those sources into 16 deterministic replay outcome fixtures with validation and safety validation (no network, no Solana RPC, no provider APIs, no live data, no real replay/backtesting/paper/live trading, no wallets, no execution, no investment advice, no accusations, no external network, no file export/download behavior)
