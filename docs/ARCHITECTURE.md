...
- `offline-intelligence`: Phase 33 composite offline-intelligence fixture-model layer; combines synthetic cross-references to Phase 30/31/32 fixture outputs into 16 unified composite evidence fixtures with multi-source risk/quality/confidence indicators, weighting, summary, validation, safety validation, and builders (no network, no Solana RPC, no provider APIs, no live data, no wallets, no Jito/MEV/mempool, no execution, no investment advice, no accusations, no external network)
