...
- `offline-intelligence`: Phase 33 composite fixture-model layer + Phase 34 report-integration layer + Phase 36 replay-outcome fixture layer + Phase 37 score-band outcome analysis layer + Phase 38 strategy-candidate evaluation fixture layer; combines synthetic cross-references to prior fixture outputs into deterministic composite/report/replay/score-band/strategy-candidate fixtures with validation and safety validation (no network, no Solana RPC, no provider APIs, no live data, no real scoring/ranking/replay/backtesting/paper/live trading, no wallets, no execution, no investment advice/recommendations/signals, no accusations, no external network, no file export/download behavior)
