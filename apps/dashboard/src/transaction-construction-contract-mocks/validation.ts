import {
  ...
  type TransactionConstructionContractMockSafetyResult,
  type TransactionConstructionContractMockValidationIssue,
  type TransactionConstructionContractMockValidationResult,
} from './types.js';
import {
...
function pushIssue(
  issues: TransactionConstructionContractMockValidationIssue[],
  code: string,
  field: string,
  message: string,
...
function scanText(value: string, field: string, issues: TransactionConstructionContractMockValidationIssue[]): void {
...
export function validateTransactionConstructionContractMockFixture(
  fixture: TransactionConstructionContractMockFixture,
): TransactionConstructionContractMockValidationResult {
  const issues: TransactionConstructionContractMockValidationIssue[] = [];

  if (!fixture.fixtureId) pushIssue(issues, 'MISSING_FIXTURE_ID', 'fixtureId', 'fixtureId is required.');
  if (!isValidTransactionConstructionContractMockName(fixture.fixtureName)) pushIssue(issues, 'INVALID_FIXTURE_NAME', 'fixtureName', 'fixtureName is invalid.');
  if (!isValidTransactionConstructionContractMockKind(fixture.fixtureKind)) pushIssue(issues, 'INVALID_FIXTURE_KIND', 'fixtureKind', 'fixtureKind is invalid.');
  if (fixture.phase !== TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_PHASE) pushIssue(issues, 'WRONG_PHASE', 'phase', `phase must be ${TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_PHASE}.`);
  if (!isValidTransactionConstructionContractMockSchemaVersion(fixture.schemaVersion)) pushIssue(issues, 'INVALID_SCHEMA_VERSION', 'schemaVersion', 'schemaVersion is invalid.');
  if (!isValidTransactionConstructionContractMockGeneratedAt(fixture.meta.generatedAt)) pushIssue(issues, 'INVALID_GENERATED_AT', 'meta.generatedAt', 'generatedAt is invalid.');
  if (!isValidTransactionConstructionContractMockSource(fixture.meta.source)) pushIssue(issues, 'INVALID_SOURCE', 'meta.source', 'source is invalid.');
...
export function validateTransactionConstructionContractMockSafety(
  fixture: TransactionConstructionContractMockFixture,
): TransactionConstructionContractMockSafetyResult {
...
export function validateTransactionConstructionContractMockFixtureTable(
  fixtures: readonly TransactionConstructionContractMockFixture[],
): TransactionConstructionContractMockValidationResult {
  const issues: TransactionConstructionContractMockValidationIssue[] = [];
  ...
  for (const fixture of fixtures) {
    const result = validateTransactionConstructionContractMockFixture(fixture);
