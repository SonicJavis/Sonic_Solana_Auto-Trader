...
import { getTransactionConstructionContractMockCapabilities } from './capabilities.js';
...
import { stableDeterministicTransactionConstructionContractMockChecksum } from './normalization.js';
...
import {
  ...
  PHASE_82_TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_VERSION,
  type BuildTransactionConstructionContractMockFixtureInput,
...
const BLUEPRINTS: Record<TransactionConstructionContractMockName, Blueprint> = {
  'mock-construction-contract-ready': {
    fixtureKind: 'mock_construction_contract_ready',
    gateStatus: 'ready',
  ...
  'missing-execution-boundary-blocked': {
    fixtureKind: 'missing_execution_boundary_blocked',
    gateStatus: 'blocked',
    stateKind: 'blocked',
    stateStatus: 'blocked',
  ...
  'real-transaction-object-rejected': {
    fixtureKind: 'real_transaction_object_rejected',
    gateStatus: 'blocked',
    stateKind: 'blocked',
    stateStatus: 'blocked',
    approvalStatus: 'pending',
  ...
  'serialization-request-denied': {
    fixtureKind: 'serialization_request_denied',
  ...
  'unsafe-account-meta-rejected': {
    fixtureKind: 'unsafe_account_meta_rejected',
    gateStatus: 'review_required',
    stateKind: 'design_only',
    stateStatus: 'design_ready',
...
export function buildTransactionConstructionContractMockFixture(
  input: BuildTransactionConstructionContractMockFixtureInput,
): TransactionConstructionContractMockFixture {
  const fixtureName = input.fixtureName;
  const blueprint = BLUEPRINTS[fixtureName];
  const sourceIndex = TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_PHASE + Object.keys(BLUEPRINTS).indexOf(fixtureName);
  const fixtureId = `phase82-${fixtureName}`;
  ...
  const boundaryGate = buildExecutionBoundaryGate({
    ...
    boundaryGateName: `phase82-gate-${fixtureName}`,
  ...
  const dryRunLinkage = buildExecutionDryRunLinkage({
    ...
    linkageStatus: fixtureName === 'missing-execution-boundary-blocked' ? 'blocked' : blueprint.linkageStatus,
  });
  ...
  const readinessLinkage = buildExecutionReadinessLinkage({
    ...
    linkageStatus: fixtureName === 'real-transaction-object-rejected' ? 'blocked' : blueprint.linkageStatus,
  });
  ...
  const evidenceBundle = buildExecutionBoundaryEvidence({
    ...
    safetyGrepRefs: [
      'rg "privateKey|seedPhrase|mnemonic|Keypair|signTransaction|sendTransaction" apps/dashboard/src/transaction-construction-contract-mocks tests/phase82.test.ts docs/TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS.md',
      'rg "execute|trade|buy|sell|order|dispatch|wallet|fs\.|writeFile|setInterval|setTimeout|route|handler|server|listen" apps/dashboard/src/transaction-construction-contract-mocks',
    ],
    docsRefs: [
  ...
  const capabilities = getTransactionConstructionContractMockCapabilities();
  ...
  return {
    ...
    schemaVersion: PHASE_82_TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_SCHEMA_VERSION,
    boundaryGate,
    boundaryState,
    constructionDenial,
    signingDenial,
    ...
    meta: {
      generatedAt: PHASE_82_TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_GENERATED_AT,
      source: PHASE_82_TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_SOURCE,
      version: PHASE_82_TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_VERSION,
      phase: TRANSACTION_CONSTRUCTION_CONTRACT_MOCKS_PHASE,
      deterministicSeed: stableDeterministicTransactionConstructionContractMockChecksum(checksumInput),
    },
    safety: {
      fixtureOnly: true,
      localOnly: true,
